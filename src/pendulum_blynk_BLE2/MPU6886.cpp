...
int MPU6886::Init(void) {
  ...
  regdata = 0x18; //FCHOICE_B = 0b00
  I2C_Write_NBytes(MPU6886_ADDRESS, MPU6886_GYRO_CONFIG, 1, &regdata);
  delay(1);

  regdata = 0x01; //ジャイロセンサ LPF 176Hz
  I2C_Write_NBytes(MPU6886_ADDRESS, MPU6886_CONFIG, 1, &regdata);
  delay(1);

  //regdata = 0x05; //サンプリングレート 1k/(1+5) = 166Hz
  regdata = 0x00; //サンプリングレート 1k/(1+0) = 1kHz
...
//!俯仰，航向，横?：pitch，yaw，roll，指三?空?中?行器的旋?状?。
...
{
  //return IIC_Write_Byte(MPU_GYRO_CFG_REG,scale<<3);//?置陀螺??量程范?
  unsigned char regdata;
  regdata = (scale << 3);
  I2C_Write_NBytes(MPU6886_ADDRESS, MPU6886_GYRO_CONFIG, 1, &regdata);
  delay(10);
